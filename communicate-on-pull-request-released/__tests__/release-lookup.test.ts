...
describe('release lookup via GitHub Releases API', () => {
  it('uses listReleases when version is found and performs all side effects', async () => {
    ...
    const api = nock('https://api.github.com')
      ...
      .post(
        ...
        JSON.stringify({
          body: 'Congratulations! :tada: This was released as part of [_fastlane_ 3.0.0](https://github.com/foo/bar/releases/tag/3.0.0) :rocket:',
          event: 'COMMENT'
        })
      )
      .reply(200)
      ...
      .reply(200, [])
      .post('/repos/foo/bar/issues/999/labels', JSON.stringify({labels: ['label-to-add']}))
      .reply(200)
      ...
      .post(
        ...
        JSON.stringify({
          body: 'The pull request #999 that closed this issue was merged and released as part of [_fastlane_ 3.0.0](https://github.com/foo/bar/releases/tag/3.0.0) :rocket:\nPlease let us know if the functionality works as expected as a reply here. If it does not, please open a new issue. Thanks!'
        })
      )
      .reply(200);
  ...
  it('exits early when version is not found in listReleases and no payload fallback is available', async () => {
    ...
    const releases = [{name: '1.0.0 Improvements'}, {name: '3.0.0 Improvements'}];
